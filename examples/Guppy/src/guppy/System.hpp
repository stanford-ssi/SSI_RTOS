...
public:
    ...
    public:
        ...
        // GPSTask gps = GPSTask(2);
        TelemetryTask telem = TelemetryTask(3);
        ArmingTask arm = ArmingTask(4);
        RadioTask radio = RadioTask(5);
        AltimeterTask alt = AltimeterTask(6); //monitors system health
        SensorTask sensor = SensorTask(7); //reads data from sensors
        AltFilterTask filter = AltFilterTask(8); //KF for altitude estimations
    };
