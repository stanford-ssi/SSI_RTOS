...
void FlightPlan::update(AltFilter& filter, uint32_t taskid){
    ...
    case Waiting:
        bool iscurarmed;
        sys.armed.get(iscurarmed, taskid);
        if(state_timer > 2000 && velocity > 100.0 && (bool) iscurarmed){ //launch!
            state = Flight;
            state_timer = 0;
            sys.pyro.arm();
            logState(taskid);
            filter.logState(taskid);
        }else{
            pad_alt_counter++;
        ...
        if(event_done[i] == false){
               ...
               ((e.altCond == AltNone) || (e.altCond == AltLess && altitude < e.altitude) || (e.altCond == AltMore && altitude > e.altitude)))   //check altitude threshold
            {
                if(filter_time > event_timer){
                    ...
                    // sys.tasks.logger.logJSON(event_json, "event", taskid);
                }
            }   
        }
    }
...
void FlightPlan::logState(uint32_t taskid){
    p_state.post(state, taskid);
    pyroA_fired.post(squibAFired, taskid);
    pyroB_fired.post(squibBFired, taskid);
    pyroC_fired.post(squibCFired, taskid);
    pyroD_fired.post(squibDFired, taskid);
    ...
    // sys.tasks.logger.logJSON(json, "flight_state", taskid);
}

//TODO: Strongly Typed Enums, and an implicit conversion to strings for logging
void FlightPlan::dumpConfig(uint32_t taskid){
    ...
    // sys.tasks.logger.logJSON(flightplan_json, "flightplan", taskid);
}