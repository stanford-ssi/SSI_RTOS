...
SensorTask::SensorTask(uint8_t priority) : Task(priority) {}
...
void SensorTask::activity()
{
    ...
    sys.sensors.adxl1.init(xGetTaskId());
    sys.sensors.adxl1.startMeasuring();

    vTaskDelay(2); //but why...

    sys.sensors.adxl2.init(xGetTaskId());
    sys.sensors.adxl2.startMeasuring();

    vTaskDelay(2); //but why...

    rc = sys.sensors.pres1.init(xGetTaskId());
    ...
    rc = sys.sensors.pres2.init(xGetTaskId());
    ...
    uint32_t lastSensorTime = xTaskGetTickCount();
    ...
    while (true)
    {
        ...
        sys.tasks.filter.queueSensorData(data, xGetTaskId());
        ...
        sys.tasks.logger.logJSON(sensor_json, "sensor", xGetTaskId());
        // Serial.println("post log sensor");
        // char buf[100];
        // snprintf(buf, sizeof(buf), "__01234567890123456789012345678901234567890123456789012345678901234567890123456789__");
        // sys.tasks.logger.log(buf, xGetTaskId());

        // Serial.println(data.adxl1_data.y);
