...
void AltFilter::update(SensorData& data, uint32_t taskid){
  kalmanPredict();
  prefilter(data);
  ...
  p_alt.post(X(0), taskid);
  p_vel.post(X(1), taskid);
}
...
void AltFilter::logState(uint32_t tid){
  ...
  z_json.add(Z(1));
  if (!sys.shitl) {
    sys.tasks.logger.logJSON(json, "filter_state", tid);
  }
}
